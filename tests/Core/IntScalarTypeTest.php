...
beforeEach(function () {
    $this->scalar = new IntScalarType();
});

it('can serialize numerics', function () {
    expect($this->scalar->serialize(123))->toBe(123);
    expect($this->scalar->serialize(123.45))->toBe(123);
    expect($this->scalar->serialize(123.99))->toBe(123); // Ensure that we're rounding down
    expect($this->scalar->serialize('123'))->toBe(123);
});
...
it('can parse numerics', function () {
    ...
    expect($this->scalar->parseValue(123.45))->toBe(123);
    expect($this->scalar->parseValue('123'))->toBe(123);
});
...
it('can parse literal stringables', function () {
    ...
    expect($this->scalar->parseLiteral($node, null))->toBe(123);
});
